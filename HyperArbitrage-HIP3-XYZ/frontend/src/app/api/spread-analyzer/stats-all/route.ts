...
function computeMR(values: number[], totalFeeBps: number) {
...
export async function GET(request: NextRequest) {
  ...
  if (!state || !state.pairs) {
    return NextResponse.json({ error: "Collector not ready" }, { status: 503 });
  }
  ...
  try {
    ...
    const spreadsByPair: Record<string, { d1: number[]; d2: number[] }> = {};
    ...
    const mrMetrics: Record<string, any> = {};
