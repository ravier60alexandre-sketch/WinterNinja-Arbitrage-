...
export async function GET(request: NextRequest) {
  ...
  try {
    const rows = database
      .prepare(
        `SELECT * FROM spread_observations WHERE pair_a = ? AND pair_b = ? AND timestamp >= ? ORDER BY timestamp ASC LIMIT ?`
      )
      .all(pairA, pairB, since, limit);

    return NextResponse.json({ pair_a: pairA, pair_b: pairB, window: windowParam, count: rows.length, data: rows });
  } catch (err: any) {
