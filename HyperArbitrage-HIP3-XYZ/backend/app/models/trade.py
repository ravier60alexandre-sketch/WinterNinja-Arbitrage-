...
from sqlalchemy import BigInteger, DateTime, ForeignKey, Index, Integer, Numeric, String, func, text
...
class Trade(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_trades_bot_open", "bot_id", "status", postgresql_where=text("status = 'open'")),
    )
    ...
    exit_price_a: Mapped[Decimal | None] = mapped_column(Numeric(20, 8), nullable=True)
    exit_price_b: Mapped[Decimal | None] = mapped_column(Numeric(20, 8), nullable=True)
    ...
    funding_paid: Mapped[Decimal] = mapped_column(Numeric(20, 8), server_default="0")
    ...
    entry_spread: Mapped[Decimal | None] = mapped_column(Numeric(20, 8), nullable=True)
    exit_spread: Mapped[Decimal | None] = mapped_column(Numeric(20, 8), nullable=True)
    edge_at_entry: Mapped[Decimal | None] = mapped_column(Numeric(20, 8), nullable=True)
...
class SpreadSnapshot(Base):
    ...
    __table_args__ = (
        Index("ix_snapshots_bot_ts", "bot_id", text("timestamp DESC")),
    )
